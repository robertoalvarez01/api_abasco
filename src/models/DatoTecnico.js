...
class DatoTecnicoModel{
    ...
    update(body,id){
        return new Promise((resolve,reject)=>{
            db.query("UPDATE datos_tecnicos SET  dormitorios =?, s_terreno =?, s_cubierta =?, s_semicubierta =?, s_total=?, cochera =?, pileta =?, u_medida = ? WHERE idCasa = ?",[body.dormitorios,body.s_terreno,body.s_cubierta,body.s_semicubierta,body.s_total,body.cochera,body.pileta,body.u_medida,id],(err,res,fields)=>{
                if(err) throw err;
                resolve(res);
            })
        })
